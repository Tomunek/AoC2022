...
def main():
    ...
    free_space = 70000000 - root_dir.get_size()
    small_deletable = 70000000
    required_space = 30000000 - free_space
    for size in sizes:
        if required_space <= size < small_deletable:
            small_deletable = size

    print(f"Size of smallest directory we have to delete: {small_deletable}")
